%% STR-dynamic-Indirect feedback-pole placemnet-nozero cancelled_martabe1
clc
clear all
close all
syms q
...
%% sys identify martabe 1
phi_H= zeros(1,2);

k = zeros(length(phi_H(1,:)));
p = zeros(length(phi_H(1,:)));
alpha =1e2;
p(:,:,1) = alpha*eye(length(phi_H(1,:)));
teta_H = [zeros(2,1),zeros(2,1)];
phi_H= zeros(1,2);
...
%% testing controller obtaiend parameters Ru=TUc-Sy
   ...
   yh(i)=-(Ah(1,2)*yh(i-1))-Ah(1,3)*yh(i-2)-(Ah(1,4)*yh(i-3))+(Bh(1,2)*Uc(i-1))+(Bh(1,3)*Uc(i-2))+(Bh(1,4)*Uc(i-3));
    ...
    Y(i)=-(teta_H(1,i)*Y(i-1))+(teta_H(2,i)*U(i-1));
...
%% plotting
 ...
 plot(2.3*Y,'g-','linewidth',3);hold on
 plot(2.5*yg,'r--','linewidth',2.5);legend('Uc','Y','Yd');grid on
subplot(2,1,2);
