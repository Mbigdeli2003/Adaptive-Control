%% STR Direct_nozero pole cancelled-martabe2
clc
clear all
close all
syms q
...
k_2 = zeros(length(phi_2(1,:)));
p_2 = zeros(length(phi_2(1,:)));
alpha =1e10;
p_2(:,:,5) = alpha*eye(length(phi_2(1,:)));
teta_2 = [zeros(4,1),zeros(4,1),zeros(4,1),zeros(4,1),zeros(4,1)];
phi_2= zeros(1,4);
...
alpha =1e10;
P_2(:,:,5) = alpha*eye(length(phi_2(1,:)));
teta_H_2 = [zeros(4,1),zeros(4,1),zeros(4,1),zeros(4,1),zeros(4,1)];
A_2=zeros(1,3);
B_2=zeros(1,3);
...
phi_2(i,:)=[-Y(i-1) -Y(i-2) U(i-1) U(i-2)];
    k_2(:,i)=p_2(:,:,i-1)*phi_2(i,:)'/(1+phi_2(i,:)*p_2(:,:,i-1)*phi_2(i,:)');
    p_2(:,:,i)=(eye(length(phi_2(i,:)))-k_2(:,i)*phi_2(i,:))*p_2(:,:,i-1);
    teta_2(:,i)=teta_2(:,i-1)+k_2(:,i)*(Y(i)-phi_2(i,:)*teta_2(:,i-1));
A_2(i,1:2)=teta_2(1:2,i);
B_2(i,1:2)=teta_2(3:4,i);

 Y_2(i)=-(A_2(i,1)*Y_2(i-1))-(A_2(i,2)*Y_2(i-2))+(B_2(i,1)*U(i-1))+(B_2(i,2)*U(i-2));


%  Y(i)=R(1,1)*Uf(i-1)+R(1,2)*Uf(i-2)+R(1,3)*Uf(i-3)+S(1,1)*Uf(i-1)+S(1,2)*Uf(i-2)+S(1,3)*Uf(i-3);
end
SSE=0.01*((sum(4*Y_2-Uc))^2)
%% plotting
 figure;
 subplot(2,1,1)
 plot(Uc,'b','linewidth',3);axis([0 500 -4 6]);hold on
 plot(2.5*Y,'g-','linewidth',1.8);axis([0 500 -4 6]);hold on
 plot(4*Y_2,'y--','linewidth',3);axis([0 500 -4 6]);hold on
 plot(2.5*yg,'r--','linewidth',2.5);legend('Uc','Y','Y_2','Yd');axis([0 500 -4 6]);grid on
subplot(2,1,2);
