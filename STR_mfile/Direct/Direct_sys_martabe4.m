%% STR Direct_nozero pole cancelled-martabe4
clc
clear all
close all
syms q
...
%% system identifying martabe 3 for the controller
phi_H= zeros(1,6);
...
k_4 = zeros(length(phi_4(1,:)));
p_4 = zeros(length(phi_4(1,:)));
alpha =1e10;
p_4(:,:,5) = alpha*eye(length(phi_4(1,:)));
teta_4 = [zeros(8,1),zeros(8,1),zeros(8,1),zeros(8,1),zeros(8,1)];
phi_4= zeros(1,8);
...
alpha =1e10;
P_4(:,:,5) = alpha*eye(length(phi_4(1,:)));
teta_H_4 = [zeros(8,1),zeros(8,1),zeros(8,1),zeros(8,1),zeros(8,1)];
A_4=zeros(1,4);
B_4=zeros(1,4);
...
for i=6:length(Uc)
  ...
  if i<15
    Y(i)=-(Ah(1,2)*Y(i-1))-(Ah(1,3)*Y(i-2))-(Ah(1,4)*Y(i-3))+(Bh(1,2)*Uc(i-1))+(Bh(1,3)*Uc(i-2))+(Bh(1,4)*Uc(i-3));
  phi_H(i,:)=[-Y(i-1) -Y(i-2) -Y(i-3) Uc(i-1) Uc(i-2) Uc(i-3)];
  elseif i>=15
    
       Y(i)=-(A(i-1,1)*Y(i-1))-(A(i-1,2)*Y(i-2))-(A(i-1,3)*Y(i-3))+(B(i-1,1)*U(i-1))+(B(i-1,2)*U(i-2))+(B(i-1,3)*U(i-3)); 
 phi_H(i,:)=[-Y(i-1) -Y(i-2) -Y(i-3) U(i-1) U(i-2) U(i-3)];
  end
...
phi_4(i,:)=[-Y(i-1) -Y(i-2) -Y(i-3) -Y(i-4) U(i-1) U(i-2) U(i-3) U(i-4)];
    k_4(:,i)=p_4(:,:,i-1)*phi_4(i,:)'/(1+phi_4(i,:)*p_4(:,:,i-1)*phi_4(i,:)');
    p_4(:,:,i)=(eye(length(phi_4(i,:)))-k_4(:,i)*phi_4(i,:))*p_4(:,:,i-1);
    teta_4(:,i)=teta_4(:,i-1)+k_4(:,i)*(Y(i)-phi_4(i,:)*teta_4(:,i-1));
A_4(i,1:4)=teta_4(1:4,i);
B_4(i,1:4)=teta_4(5:8,i);

 Y_4(i)=-(A_4(i,1)*Y_4(i-1))-(A_4(i,2)*Y_4(i-2))-(A_4(i,3)*Y_4(i-3))-(A_4(i,4)*Y_4(i-4))+(B_4(i,1)*U(i-1))+(B_4(i,2)*U(i-2))+(B_4(i,3)*U(i-3))+(B_4(i,4)*U(i-4));


%  Y(i)=R(1,1)*Uf(i-1)+R(1,2)*Uf(i-2)+R(1,3)*Uf(i-3)+S(1,1)*Uf(i-1)+S(1,2)*Uf(i-2)+S(1,3)*Uf(i-3);
end
%% plotting
SSE=0.01*(sum(2.5*Y_4-Uc))^2
 figure;
 subplot(2,1,1)
 plot(Uc,'b','linewidth',3);axis([0 500 -4 6]);hold on
 plot(2.5*Y,'g-','linewidth',1.8);axis([0 500 -4 6]);hold on
 plot(2.5*Y_4,'y--','linewidth',3);axis([0 500 -4 6]);hold on
 plot(2.5*yg,'r--','linewidth',2.5);legend('Uc','Y','Y_4','Yd');axis([0 500 -4 6]);grid on
subplot(2,1,2);
