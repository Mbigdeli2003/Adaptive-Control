%% Exer_2_adaptive_online_coloured noise
clc
clear all
close all
format long
...
%% adaptive identification system H RLS
for i=4:1:max(size(u_1))
 yh(i)=-(Ah(1,2)*yh(i-1))-Ah(1,3)*yh(i-2)-(Ah(1,4)*yh(i-3))+(Bh(1,2)*u_1(i-1))+(Bh(1,3)*u_1(i-2))+(Bh(1,4)*u_1(i-3))+c(1,1)*e(i-1)+c(1,2)*e(i-2)+c(1,3)*e(i-3);   
...
alpha =1e6;
p(:,:,3) = alpha*eye(length(phi_H(1,:)));
teta_H = [zeros(9,1),zeros(9,1),zeros(9,1)];
% error = sum((y-y_hat(:,length(y))).^2)

for i=4:1:max(size(u_1))
 yh(i)=-(Ah(1,2)*yh(i-1))-Ah(1,3)*yh(i-2)-(Ah(1,4)*yh(i-3))+(Bh(1,2)*u_1(i-1))+(Bh(1,3)*u_1(i-2))+(Bh(1,4)*u_1(i-3))+c(1,1)*e(i-1)+c(1,2)*e(i-2)+c(1,3)*e(i-3);   
  phi_H(i,:)=[-yh(i-1) -yh(i-2) -yh(i-3) u_1(i-1) u_1(i-2) u_1(i-3) e(i-1) e(i-2) e(i-3)];
    k(:,i)=p(:,:,i-1)*phi_H(i,:)'/(1+phi_H(i,:)*p(:,:,i-1)*phi_H(i,:)');
    p(:,:,i)=(eye(length(phi_H(i,:)))-k(:,i)*phi_H(i,:))*p(:,:,i-1);
    teta_H(:,i)=teta_H(:,i-1)+k(:,i)*(yh(i)-phi_H(i,:)*teta_H(:,i-1));
